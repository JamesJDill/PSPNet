...
class PSPNet(nn.Module):
    ...
    def __create_classifier(self, in_feats: int, out_feats: int, num_classes: int) -> nn.Module:
        ...
        """
        cls = nn.Sequential(
            nn.Conv2d(in_channels=in_feats, out_channels=out_feats, kernel_size=3, padding=1, bias=False),
    ...
    def forward(
        ...
        H = int(self.zoom_factor * (x_size[2] - 1) / 8 + 1)
        W = int(self.zoom_factor * (x_size[3] - 1) / 8 + 1)
        ...
        yhat = F.softmax(logits, dim=1).argmax(dim=1)
        ...
        if self.training:
            logits_aux = self.aux(x_layer3)
            logits_aux = F.interpolate(logits_aux, size=(H,W), mode='bilinear', align_corners=True)
            aux_loss = self.criterion(logits_aux, y)
