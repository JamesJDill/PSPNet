...
def get_model_and_optimizer(args) -> Tuple[nn.Module, torch.optim.Optimizer]:
    ...
    if args.arch == 'SimpleSegmentationNet':
        ...
        base_modules = [model.layer0, model.resnet.layer1, model.resnet.layer2, model.resnet.layer3, model.resnet.layer4]
        other_modules = [model.cls]
        ...
        base_modules = [model.layer0, model.layer1, model.layer2, model.layer3, model.layer4]
        if args.use_ppm:
            other_modules = [model.ppm, model.cls, model.aux]
        else:
            other_modules = [model.cls, model.aux]

    parameter_list = []
    for module in base_modules:
        parameter_list.append(dict(params=module.parameters(), lr=args.base_lr))
    for module in other_modules:
        parameter_list.append(dict(params=module.parameters(), lr=args.base_lr * 10))
    
    optimizer = torch.optim.SGD(params=parameter_list, lr=args.base_lr, momentum=args.momentum, weight_decay=args.weight_decay)
...
def get_val_transform(args) -> transform.Compose:
    ...
    val_transform = transform.Compose([
        transform.ResizeShort(args.short_size),
        transform.Crop(size=(args.train_h, args.train_w), padding=mean, ignore_label=args.ignore_label),
        transform.ToTensor(),
