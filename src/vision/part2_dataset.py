...
def make_dataset(split: str, data_root: str, data_list_fpath: str) -> List[Tuple[str, str]]:
    ...
    with open(data_list_fpath, 'r') as file:
        ...
        for line in lines:
            rel_image_path, rel_label_path = line.strip().split()
            abs_image_path = data_root + '/' + rel_image_path # using os.path.join was concatenating with '\' which was failing the local test
            abs_label_path = data_root + '/' + rel_label_path # im asusming this is because im on windows and the TA who made these local tests is on mac
            #abs_image_path = os.path.join(data_root, rel_image_path)
            #abs_label_path = os.path.join(data_root, rel_label_path)
            image_label_list.append((abs_image_path, abs_label_path))
